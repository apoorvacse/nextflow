...
function normalizeGeminiModel(model) {
  ...
  return lower.replace(/\./g, "-").replace(/\s+/g, "-").replace(/-+$/g, "");
}
...
__name(normalizeOpenRouterModel, "normalizeOpenRouterModel");
async function runWithOpenRouter(payload) {
  ...
  const call = /* @__PURE__ */ __name(async (model) => {
  ...
  for (const model of modelFallbacks) {
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const isModelNotFound = res.status === 404 || String(msg).toLowerCase().includes("model") && String(msg).toLowerCase().includes("not found");
      if (isModelNotFound) {
...
__name(runWithOpenRouter, "runWithOpenRouter");
var llmTask = task({
  id: "llm-node",
  retry: { maxAttempts: 2, minTimeoutInMs: 1e3 },
  run: /* @__PURE__ */ __name(async (payload) => {
    const { model, systemPrompt, userMessage, imageUrls } = payload;
    if (process.env.OPENROUTER_API_KEY) {
      const response2 = await runWithOpenRouter(payload);
    ...
    for (const modelId of fallbackModels) {
      try {
        ...
        const isModelNotFound = msg.includes("404 Not Found") || msg.includes("not found for API version") || msg.includes("not supported for generateContent");
...
@google/generative-ai/dist/index.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-2DM4HGGE.mjs.map
